...
def get_values(task, x, ys, n_generate_sample, llama, evaluation_config):
    ...
    for batch in itertools.zip_longest(*[iter(ys)] * n_batch, fillvalue=None):
        value_prompts = [task.value_prompt_wrap(x, y) for y in batch if y is not None]

        _inputs = llama.tokenizer(value_prompts, return_tensors="pt", padding=True, truncation=True)
        ...
        for output in value_outputs:
            output["text"] = llama.tokenizer.decode(
                llama.tokenizer.encode(output["text"], add_special_tokens=False),
                skip_special_tokens=True,
                clean_up_tokenization_spaces=True
            ).strip()
            probability, invalid = task.value_outputs_unwrap([output])  # or output
